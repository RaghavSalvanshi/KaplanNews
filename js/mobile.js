...
/* Comment Generated by Combres - Resource '~/scripts/jquery-1.6.4.min.js' (Mode: Static) */
...
/* Comment Generated by Combres - Resource '~/scripts/jquery-mobile-config.js' (Mode: Static) */
//Jquery Mobile Global Configuration
...
$('#page').live('pagecreate', function () {
	// Has scroll
	// Fix to enable auto scroll to hash when page loads
	$(document).bind("silentscroll", function (e) {
...
var ourSilentScroll = function (el) {
	$('html, body').animate({
...
/* Comment Generated by Combres - Resource '~/scripts/plugins/jquery.cookie.js' (Mode: Static) */
/**
...
/* Comment Generated by Combres - Resource '~/scripts/image-handling.js' (Mode: Static) */
$(function () {
...
/* Comment Generated by Combres - Resource '~/scripts/common.js' (Mode: Static) */
$('#page').live('pagecreate', function () {
	$('.search-button-glass').live('click', function () { $(this).closest('form').submit(); });
});

//fixCollapsibleSets()
$(function() {
	var parent = $("div.collapsible-set");

	$("h1.collapsible-section-start").each(function () {
		var headline = $(this);
		var content = headline.nextUntil("h1.collapsible-section-start, div[data-role='collapsible']").andSelf();
		var container = $("<div data-role='collapsible' data-collapsed='true' data-theme='m' />");
...
/* Comment Generated by Combres - Resource '~/scripts/show-more.js' (Mode: Static) */
/* Takes care of showing more items and updating the text of the button */

var showMoreList = function (button, items, count) {

	var buttontext = button.find(".ui-btn-text"), text = buttontext.text();
	...
	function updateShowMoreText() {
		...
		buttontext.text(text.replace("{0}", items.length > count ? count : items.length));
	}
	...
	function showMore() {
		...
		if (count > items.length) {
			els = $(items.splice(0, items.length));
		} else {
			els = $(items.splice(0, count));
		}

		els.show();
		els.find(".lazy-load").trigger("lazy-load");

		updateShowMoreText();
	};

	button.live("click", showMore);

	showMore();
};;


/* Comment Generated by Combres - Resource '~/scripts/dealer-cookie.js' (Mode: Static) */
$(function () {
	$(".make-preferred").live("click", function () {
		setCookie(this);
		setPreferredStyleAndText(this);
	});
});

var setCookie = function (e) {

	if ($(e).hasClass('preferred-dealer'))
		$.cookie('DealerPreferred', null, { expires: 7, path: '/' });
	else {
		$.cookie('DealerPreferred', $(e).attr("data-dealerid"), { expires: 7, path: '/' });
	}
};

var setPreferredStyleAndText = function (e) {
	return setPreferredStyleAndText(e, false);
};

var setPreferredStyleAndText = function (e, clear) {

	var preferred = true;
	if ($(e).hasClass('preferred-dealer'))
		preferred = false;

	// Remove existing styles and texts
	$('.make-preferred').removeClass('preferred-dealer');
	$('.make-preferred').each(
		function () {
			$(this).find('.ui-btn-text').text($(this).attr('data-preftext'));
		}
	);

	// Set new style and texts
	if (preferred && !clear) {
		$(e).addClass('preferred-dealer');
		$(e).find('.ui-btn-text').text($(e).attr('data-unpreftext'));
	}
	else {
		$(e).find('.ui-btn-text').text($(e).attr('data-preftext'));
	}
};;


/* Comment Generated by Combres - Resource '~/scripts/dealer-fetcher.js' (Mode: Static) */
/* Gets dealers by gps location
*/
var nearestDealers = function (brand, nodeProgId) {

	var dealersLoaded = function (userLocation, dealers, isMiles, callback) {
	...
	var loadDealers = function (callback, error, pageSize, isMiles) {

		if (navigator.geolocation) {

			navigator.geolocation.getCurrentPosition(function (position) {

				var distanceUnit = isMiles ? "m" : "k";

				var url = 'http://rest.dim.husqvarna.com/rest/v2/getdealersByLocation.ashx?pageNumber=0&pageSize=' + pageSize + '&brand=' + brand + '&nodeprogid=' + nodeProgId + '&latitude=' + position.coords.latitude + '&longitude=' + position.coords.longitude + '&distanceUnit=' + distanceUnit + '&pids=Phone_Shop';

				$.ajax({
					url: url,
					dataType: "jsonp",
					success: function (data) {
						dealersLoaded(position, data, isMiles, callback);
					}
				});
	...
	var getDealers = function (callback, error, pageSize, isMiles) {
		loadDealers(callback, error, pageSize, isMiles);
	};

	var getDealer = function (dealerid, callback, error, isMiles) {

		var url = 'http://rest.dim.husqvarna.com/rest/v2/getdealer.ashx?dealer=' + dealerid + '&brand=' + brand + '&pids=Phone_Shop';
...
/* Comment Generated by Combres - Resource '~/scripts/dealer-fetcher-ui.js' (Mode: Static) */
function appendDropdown(dropdown, container) {
	var size = container.find(".ui-field-contain").size();
	dropdown.wrap('<div data-role="fieldcontain"/>');

	// If change was on first dropdown we want to replace the second and not display a third ..
	if (size == 2)
		container.children().next().replaceWith(dropdown.parent());
	else
		dropdown.parent().appendTo(container);
...
function renderDealers(dealers) {
	...
	dealers = updateDealers(dealers, false);
	$("#dealer-template").tmpl(dealers).appendTo("#dll-dealers-result");
	if ($.cookie('DealerPreferred')) {
		$('.make-preferred').each(function () { if ($.cookie('DealerPreferred') && $(this).attr("data-dealerid") == $.cookie('DealerPreferred')) setPreferredStyleAndText(this); });
	}
	$("#dll-dealers-result").trigger("create");
}

function updateDealers(dealers, lc) {
	...
	for (var i = 0, length = dealers.length; i < length; i++) {
		var d = dealers[i];
		console.log(d);
		d.addressDetailedString = buildAddress(d.visitAddress || d.a, lc);
...
function getPhone(dealer) {
	
	var tempProperties = [];

	$(dealer.p).each(function () {

		var p = this;

		tempProperties[p.ProgId] = p.Values[0].Value;
	});

	if (tempProperties.Phone_Shop !== undefined && tempProperties.Phone_Shop !== null && tempProperties.Phone_Shop.length > 0)
		return tempProperties.Phone_Shop;
	else
		return '';
}

function buildAddress(address, lc) {

	if(lc) {
		return formatAddress(address.address, address.city, address.postalCode, address.state);
	}
	else {
		return formatAddress(address.Address, address.City, address.PostalCode, address.State);
	}
	...
	var a = '';
	var prop = new Array("Address", "PostalCode", "City", "State", "Country");

	for (var i = 0; i < prop.length; i++) {
		var property = prop[i];
		if (lc)
			property = property[0].toLowerCase() + property.substr(1, property.length - 1);
		
		if (address[property] && address[property].length > 0)
			a += address[property] + ', ';
	}
...
function renderDealerResult(dealers, selectNearbyDealerText, distanceText, noNearbyDealersFound) {

	selectNearbyDealerText = $("<div/>").html(selectNearbyDealerText).text();
	noNearbyDealersFound = $("<div/>").html(noNearbyDealersFound).text();

	// No nearby dealers found
	if (dealers.length == 0) {
		$("#dealer-locator-light-gps p").replaceWith("<p>" + noNearbyDealersFound + "</p>");
		return;
	}

	dealers = updateDealers(dealers, true);
	$("#dealer-locator-light-gps h1").text(selectNearbyDealerText);
	$("#dealer-locator-light-gps p").remove();
	$("#dealer-template").tmpl(dealers).appendTo("#dealer-locator-light-gps").trigger("create");
	$(".dealer-collapsible-set .ui-collapsible-heading-toggle").each(function () {
		$(this).append('<span class="dealer-distance">' + $(this).parents(".ui-collapsible-heading").attr("data-distance") + ' ' + distanceText + '</span>');
	});
	if ($.cookie('DealerPreferred')) {
		$('.make-preferred').each(function () { if ($.cookie('DealerPreferred') && $(this).attr("data-dealerid") == $.cookie('DealerPreferred')) setPreferredStyleAndText(this); });
	}
	$("#loadMore").tmpl().appendTo("#dealer-locator-light-gps");
	$("#dealer-locator-light-gps").trigger("create");
};;


/* Comment Generated by Combres - Resource '~/scripts/carousel.js' (Mode: Static) */
/* Created 2011-09-28
 * by JOKN
 */
...
function Carousel() {
	var itemWidth = 240;
	var itemSpacing; // The space between items
	var carouselWidth; // The width of the <ul>, not only the visible part
	var items = [];
	var selectedIndex = 0;
	var transitionDuration = 0.3;
	var stickOutWidth = 20; // How big part of items to the left and right that will show
	var buttonFadeDuration = 200; // Next and prev button animation time
	var carousel;
	var itemCount;
	var prevButtonHidden = false;
	var nextButtonHidden = false;

	this.init = function (carouselElement, _itemWidth, stickOut, startAtIndex, transitionSpeed) {
		itemWidth = _itemWidth;
		stickOutWidth = stickOut;
		carousel = carouselElement;
		if (startAtIndex != undefined) selectedIndex = startAtIndex;
		if (transitionSpeed != undefined) transitionDuration = transitionSpeed;
		var thisCarousel = this;

		if ($.support.touch) {
			$("#prev").bind("touchstart", function () {
				thisCarousel.slidePrev();
				return false;
			});
			$("#next").bind("touchstart", function () {
				thisCarousel.slideNext();
				return false;
			});
		}
		else {
	...
	this.orderItems = function (itemCollecion) { }; // Override this function to re-order items before rendering
	
	function setBasicStyling() {
		carousel.parent().addClass("carousel-container");
		carousel.addClass("carousel");
	}

	this.render = function () {
		itemCount = Math.max(carousel.children().length, items.length);
		if (itemCount === 1) selectedIndex = 0;
		setBasicStyling();

		this.orderItems(items);

		var template = $("#carousel-item-template");
		if (template.length > 0) template.tmpl(items).appendTo(carousel);

		this.arrange();

		carousel.children().addClass("carousel-item");

		// Indicator lights
		for (var i = 0; i < itemCount; i++) {
			var li = $("<li/>");
			li.click(function () { slideToCenter((selectedIndex = $(this).index())); });
			li.append($("<a href='#'/>"));
			$("#indicators").append(li);
		}

		$("#indicators li:nth-child(" + (selectedIndex + 1) + ") a").addClass("selected");

		var transitionCssValue = "all " + transitionDuration + "s ease-in-out";

		carousel.css("-ms-transition", transitionCssValue);
		carousel.css("-webkit-transition", transitionCssValue);
		carousel.css("-moz-transition", transitionCssValue);
		carousel.css("-o-transition", transitionCssValue);
		carousel.css("transition", transitionCssValue);
	};

	function doSlide(element, x) {
		element.css("-webkit-transform", "translateX(" + x + "px)");
		element.css("-moz-transform", "translateX(" + x + "px)");
		element.css("-ms-transform", "translateX(" + x + "px)");
		element.css("-o-transform", "translateX(" + x + "px)");
		element.css("transform", "translateX(" + x + "px)");
		
		// Set indicator lights
		$("#indicators li a").removeClass("selected");
		$("#indicators li:nth-child(" + (selectedIndex + 1) + ") a").addClass("selected");
	}

	// Puts the item at the given index in center
	function slideToCenter(index) {
		var newLeft = stickOutWidth - index * (itemWidth + itemSpacing);
		doSlide(carousel, newLeft);
		
		// Show or hide buttons
		if (!nextButtonHidden && newLeft < -(carouselWidth - (itemWidth + itemSpacing) * 2)) {
			$("#next").fadeOut(buttonFadeDuration);
			nextButtonHidden = true;
		}
		else if (nextButtonHidden && newLeft >= -(carouselWidth - (itemWidth + itemSpacing) * 2)) {
			$("#next").fadeIn(buttonFadeDuration);
			nextButtonHidden = false;
		}
		if (!prevButtonHidden && newLeft > 0) {
			$("#prev").fadeOut(buttonFadeDuration);
			prevButtonHidden = true;
		}
		else if (prevButtonHidden && newLeft <= 0) {
			$("#prev").fadeIn(buttonFadeDuration);
			prevButtonHidden = false;
		}
	}
	
	function getCurrentCarouselX() {
		return selectedIndex * -(itemWidth + itemSpacing) + itemSpacing;
	}

	this.slidePrev = function () {
		// If not already at left-most position
		if (getCurrentCarouselX() < 0) {
			slideToCenter(--selectedIndex);
		}
	};

	this.slideNext = function () {
		// If not already at right-most position
		if (!nextButtonHidden) {
			slideToCenter(++selectedIndex);
		}
	};

	// This function arranges the entire carousel visually with the appropriate borderspacing and so on
	this.arrange = function () {
		itemSpacing = ($(document).width() - itemWidth - stickOutWidth * 2) / 2;
		carouselWidth = (itemCount * (itemWidth + itemSpacing) + itemSpacing);
		carousel.css("width", carouselWidth + "px");
		carousel.children().css("margin-left", itemSpacing + "px");
		slideToCenter(selectedIndex);
	};
};;


/* Comment Generated by Combres - Resource '~/scripts/add2home.js' (Mode: Static) */
/*!
...
/* Comment Generated by Combres - Resource '~/modules/dealer-locator-light/1.0/js/jquery.dealerlocatorlight.js' (Mode: Static) */
(function ($) {
	...
	var settings = {
		...
		emailText: "Email",
		faxText: "Fax",
		noDealersFoundText: "No dealers were found at this location.",
		openingHoursText: "Opening hours",
		selectText: "Select&hellip;",
		telephoneServiceText: "Telephone Service",
		telephoneShopText: "Telephone Shop",
		telephoneText: "Telephone Office",
		websiteText: "Website",
		brandId: "",
		node: "",
		dealerUrl: "",
		dealersUrl: "",
		nodesUrl: "",
		listsIdResults: "dll-lists-result",
		dealersIdContainer: "dll-dealers-result",
		renderDealers: renderDealers,
		appendDropdown: null
	};
	...
	var methods = {
		...
		init: function (options) {
			...
			return this.each(function () {
				...
				$('<div>').attr('id', settings.listsIdResults).appendTo(this);
				$('<ul>').attr('id', settings.dealersIdContainer).appendTo(this);

				loadNodesByProgId(nodesLoaded);
	...
	$.fn.dealerLocatorLight = function (method) {
		if (methods[method]) {
			return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
		} else if (typeof method === 'object' || !method) {
			return methods.init.apply(this, arguments);
		} else {
			$.error('Method ' + method + ' does not exist on jQuery.dealerLocatorLight');
			throw 'Method ' + method + ' does not exist on jQuery.dealerLocatorLight';
		}
	};

	function loadNodesByProgId(callback) {

		var url = settings.nodesUrl + "?brand=" + settings.brandId + "&node=" + settings.node;
	...
	var loadNodesByNodeId = function (nodeId, callback) {

		if (!loadNodesByNodeId.cache[nodeId]) {

			var url = settings.nodesUrl + "?brand=" + settings.brandId + "&parentnodeid=" + nodeId;
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data === null || data.length === 0) {
						...
						// no nodes found, try get dealers
						loadDealersByNodeId(nodeId, loadDealersByDealerId);

					} else {

						loadNodesByNodeId.cache[nodeId] = data;
						callback(loadNodesByNodeId.cache[nodeId], renderNodes);
	...
	// nodes by node id cache storage
	loadNodesByNodeId.cache = {};

	var loadDealersByNodeId = function (nodeId, callback) {

		if (!loadDealersByNodeId.cache[nodeId]) {

			var url = settings.dealersUrl + "?brand=" + settings.brandId + "&nodeid=" + nodeId;
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data === null || data.length === 0) {
						...
						// no dealers found, display some information message to the user
						if (settings.noDealersFoundText && settings.noDealersFoundText.length > 0) {

							$('<li class="no-dealers-found">').html(settings.noDealersFoundText).appendTo('#' + settings.dealersIdContainer);

						}

					} else {

						loadDealersByNodeId.cache[nodeId] = data;
						callback(loadDealersByNodeId.cache[nodeId], dealersLoaded);
	...
	// dealers by node id cache storage
	loadDealersByNodeId.cache = {};

	function loadDealersByDealerId(dealers, callback) {

		var j = dealers.length, tempDealers = [], url;

		$(dealers).each(function () {

			url = settings.dealerUrl + "?dealer=" + this.i;
			...
			$.ajax({
				...
				complete: function () {
					...
					if (j === 0) {
						...
						callback(tempDealers, settings.renderDealers);
	...
	function nodesLoaded(nodes, callback) {
		...
		tempNodes.sort(ascendingSorting);
	...
	function dealersLoaded(dealers, callback) {
		...
		$(dealers).each(function () {
			...
			dealer.id = this.i;
			dealer.name = this.n;
			dealer.a = this.a !== undefined ? this.a : {};
			dealer.p = this.p !== undefined ? this.p : [];

			tempDealers.push(dealer);

		});

		tempDealers.sort(ascendingSorting);

		if (callback !== null && typeof callback === "function") {

			callback(tempDealers);
	...
	function renderNodes(nodes) {
		...
		if (settings.selectText !== null && settings.selectText.length > 0) {
			content += '<option value="0">' + settings.selectText + '</option>';
		}
		...
		dropdown.change(function () {
			...
			// removing all select-siblings (and their change-events)
			$(this).nextAll().remove();

			// remove dealers
			$('#' + settings.dealersIdContainer).empty();

			if (parseInt(this.value, 10) === 0) {
				return;
			}

			loadNodesByNodeId(this.value, nodesLoaded);

		});

		if (settings.appendDropdown) {
			settings.appendDropdown(dropdown, $('#' + settings.listsIdResults));
		}
		else {
			dropdown.appendTo('#' + settings.listsIdResults);
		}
	}
	...
	function renderDealers(dealers) {
		...
		$(dealers).each(function () {
			...
			content += '<li class="vcard">';
			content += '<h3 class="org">' + d.name + '</h3>';
			if (tempProperties.Additional_Name !== undefined) {
				content += '<h4 class="org">' + tempProperties.Additional_Name + '</h4>';
			}

			// adding address
			if (d.a.Address !== undefined && d.a.Address !== null && d.a.Address.length > 0) {

				content += '<div class="adr">';

				if (settings.addressText.length > 0) {

					content += '<div class="label">' + settings.addressText + '</div>';

				}

				content += '<div class="street-address">' + d.a.Address + '</div>';

				if (d.a.PostalCode !== undefined && d.a.PostalCode !== null && d.a.PostalCode.length > 0) {

					content += '<span class="postal-code">' + d.a.PostalCode + '</span> ';

				}

				if (d.a.City !== undefined && d.a.City !== null && d.a.City.length > 0) {

					content += '<span class="locality">' + d.a.City + '</span>';
			...
			if (settings.contactInformationText.length > 0) {

				content += '<div class="label">' + settings.contactInformationText + '</div>';

			}

			// adding phone nr. shop
			if (tempProperties.Phone_Shop !== undefined && tempProperties.Phone_Shop !== null && tempProperties.Phone_Shop.length > 0) {

				content += '<div class="tel">';

				if (settings.telephoneShopText.length > 0) {

					content += '<span class="type">' + settings.telephoneShopText + '</span> ';

				}

				content += tempProperties.Phone_Shop;
				content += '</div>';

			}

			// adding phone nr. service
			if (tempProperties.Phone_Service !== undefined && tempProperties.Phone_Service !== null && tempProperties.Phone_Service.length > 0) {

				content += '<div class="tel">';

				if (settings.telephoneServiceText.length > 0) {

					content += '<span class="type">' + settings.telephoneServiceText + '</span> ';

				}

				content += tempProperties.Phone_Service;
				content += '</div>';

			}


			// adding phone nr. office
			if (tempProperties.Phone_Office !== undefined && tempProperties.Phone_Office !== null && tempProperties.Phone_Office.length > 0) {

				content += '<div class="tel">';

				if (settings.telephoneText.length > 0) {

					content += '<span class="type">' + settings.telephoneText + '</span> ';

				}

				content += tempProperties.Phone_Office;
				content += '</div>';

			}

			// adding fax nr.
			if (tempProperties.Fax !== undefined && tempProperties.Fax !== null && tempProperties.Fax.length > 0) {

				content += '<div class="tel">';

				if (settings.faxText.length > 0) {

					content += '<span class="type">' + settings.faxText + '</span> ';

				}

				content += tempProperties.Fax;
				content += '</div>';

			}

			// adding email
			if (tempProperties.Email !== undefined && tempProperties.Email !== null && tempProperties.Email.length > 0) {

				content += '<div class="email">';

				if (settings.emailText.length > 0) {

					content += '<span class="label">' + settings.emailText + '</span> ';

				}

				content += '<a href="mailto:' + tempProperties.Email + '">' + tempProperties.Email + '</a>';
				content += '</div>';

			}

			// adding website
			if (tempProperties.Website !== undefined && tempProperties.Website !== null && tempProperties.Website.length > 0) {

				content += '<div class="url">';

				if (settings.websiteText.length > 0) {

					content += '<span class="label">' + settings.websiteText + '</span> ';

				}

				content += '<a href="' + tempProperties.Website + '" target="_blank">' + tempProperties.Website + '</a>';
				content += '</div>';

			}

			if (tempProperties.Open_Hours !== undefined && tempProperties.Open_Hours !== null && tempProperties.Open_Hours.length > 0) {

				content += '<div class="operating-hours">';

				if (settings.openingHoursText.length > 0) {

					content += '<span>' + settings.openingHoursText + '</span> ';

				}

				content += tempProperties.Open_Hours;
				content += '</div>';
		...
		$(content).appendTo('#' + settings.dealersIdContainer);
...
/* Comment Generated by Combres - Resource '~/scripts/jquery.tmpl.js' (Mode: Static) */
/*
...
/* Comment Generated by Combres - Resource '~/scripts/jquery.mobile-1.0.min.js' (Mode: Static) */
